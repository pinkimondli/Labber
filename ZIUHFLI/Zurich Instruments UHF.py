...
class Driver(InstrumentDriver.InstrumentWorker):
    ...
    def performSetValue(self, quant, value, sweepRate=0.0, options={}):
		...
		elif quant.name == 'DemodAverageNum':
		    value = value
        return value
    ...
    def performGetValue(self, quant, options={}):
		...
		#Averaged trace signals
		elif quant.name == DemodAverageNum'
		    return quant.getValue()
        elif quant.name in ['TraceDemod'+str(x+1)+'R'+'Average' for x in range(8)] + \
                        ['TraceDemod'+str(x+1)+'phi'+'Average' for x in range(8)] + \
                        ['TraceDemod'+str(x+1)+'X'+'Average' for x in range(8)] + \
                        ['TraceDemod'+str(x+1)+'Y'+'Average' for x in range(8)]:
			avNumber = self.getValue(DemodAverageNum) # find out how many times you need to average
			averagedValue = 0 # initialize the final value to 0
			for i in range(avNumber): # perform the measurement several times
			    averagedValue += self.readValueFromOther(quant.name[:-8] , options={})
            averagedValue = averagedValue/avNumber # return the averaged value
